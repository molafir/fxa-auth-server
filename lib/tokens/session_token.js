...
module.exports = (log, Token, config) => {
  ...
  class SessionToken extends Token {
    ...
    constructor(keys, details) {
      if (isExpired(details)) {
        // Imitate the `Not Found` db error
  ...
  function isExpired (details) {
    if (! MAX_AGE_WITHOUT_DEVICE || details.deviceId || ! ('deviceId' in details)) {
      return false
    }

    return details.createdAt < Date.now() - MAX_AGE_WITHOUT_DEVICE
  }
}
