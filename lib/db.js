...
module.exports = (
  ...
  DB.prototype.accountRecord = function (email) {
    ...
    return this.pool.get('/email/' + Buffer(email, 'utf8').toString('hex') + '/account')
      .then(
        function (body) {
          var data = bufferize(body)

          // Ensure that the primary email address is on the account object
          // and original email is stored
          data.originalEmail = data.email

          let primaryEmail
          data.emails.some((item) => {
            if (item.isPrimary) {
              primaryEmail = item
              return true
            }
          ...
          data.normalizedEmail = primaryEmail.normalizedEmail
          data.emailCode = primaryEmail.emailCode
          data.emailVerified = !!primaryEmail.isVerified
  ...
  DB.prototype.setPrimaryEmail = function (uid, email) {
    log.trace({ op: 'DB.accountFromEmail', email: email })
    return this.pool.get('/email/' + Buffer(email, 'utf8').toString('hex') + '/account/' + uid.toString('hex'))
      .then(
        function (body) {
          var data = bufferize(body)
