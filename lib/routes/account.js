    ...
    },
    {
      ...
      config: {
        validate: {
          query: {
            keys: isA.boolean().optional(),
            service: validators.service,
            originalLoginEmail: isA.string().optional()
          },
          payload: {
            email: validators.email().required(),
            authPW: isA.string().min(64).max(64).regex(HEX_STRING).required(),
      ...
      handler: function (request, reply) {
          ...
          return db.emailRecord(email)
            .then(
              function (result) {
                ...
                // If the incorrect primary email, the password check cannot possibly succeed,
                ...
                if (! usingPrimaryEmail && request.payload.originalLoginEmail === undefined) {
                  throw error.cannotLoginWithSecondaryEmail()
                }

                if (email !== result.email && email !== result.originalEmail) {
                  throw error.incorrectPassword(result.originalEmail, email)
                }
                ...
                (code) => {
                  if (requestNow - code.createdAt > unblockCodeLifetime) {
                    log.info({
                      ...
                      uid: accountRecord.uid.toString('hex')
                    })
        ...
        function checkEmailAndPassword () {
          ...
          doSigninConfirmation = mustVerifySession && accountRecord.primaryEmail.emailVerified
          ...
          return checkPassword(accountRecord, authPW, request.app.clientAddress)
            .then(
              function (match) {
                if (! match) {
                  throw error.incorrectPassword(accountRecord.originalEmail, email)
                }
          ...
          return P.resolve()
            ...
            .then(() => {
              const sessionTokenOptions = {
                ...
                emailVerified: accountRecord.primaryEmail.emailVerified,
        ...
        function sendVerifyAccountEmail() {
          if (! accountRecord.primaryEmail.emailVerified) {
            if (didSigninUnblock) {
              log.info({
        ...
        function sendNewDeviceLoginNotification() {
          ...
          var shouldSendNewDeviceLoginEmail = config.newLoginNotificationEnabled
            ...
            && accountRecord.primaryEmail.emailVerified
          if (shouldSendNewDeviceLoginEmail) {
            const secondaryEmails = features.isSecondaryEmailEnabled(accountRecord.primaryEmail.email) ? db.accountEmails(sessionToken.uid) : P.resolve([])
    ...
    },
    {
      ...
      config: {
        ...
        validate: {
          payload: {
            email: validators.email().required(),
            authPW: isA.string().min(64).max(64).regex(HEX_STRING).optional()
          }
        },
        response: {}
      },
      handler: function (request, reply) {
        ...
        function setPrimaryEmail() {
          return db.setPrimaryEmail(uid, email.toLowerCase(), authPW)
        }
      }
    },
    {
