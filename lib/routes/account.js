          ...
          return db.emailRecord(email)
            .then(
              function (result) {
                ...
                result.emails.some((item) => {
                  if (item.normalizedEmail === normalizedEmail && item.isPrimary) {
                    usingPrimaryEmail = true
                  }
                })

                // if (! usingPrimaryEmail) {
                //   throw error.cannotLoginWithSecondaryEmail()
                // }
        ...
        function checkEmailAndPassword () {
          ...
          return checkPassword(emailRecord, authPW, request.app.clientAddress)
            .then(
              function (match) {
                if (! match) {
                  throw error.incorrectPassword(emailRecord.originalEmail, email)
                }
